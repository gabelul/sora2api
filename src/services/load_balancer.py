            ...
            for token in all_tokens:
                if token.is_active and token.expiry_time:
                    ...
                    if hours_until_expiry <= 24:
                        debug_logger.log_info(f"[LOAD_BALANCER] 🔔 Token {token.id} ({token.email}) needs refresh, remaining: {hours_until_expiry:.2f} hours")
                        refresh_count += 1
                        await self.token_manager.auto_refresh_expiring_token(token.id)

            if refresh_count == 0:
                debug_logger.log_info(f"[LOAD_BALANCER] ✅ No tokens need refresh")
            else:
                debug_logger.log_info(f"[LOAD_BALANCER] ✅ Refresh check done, checked {refresh_count} tokens")
